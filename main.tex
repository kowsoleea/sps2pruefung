...
aufweisen, eine Prüfung zur KinderpflegerIn ablegen.%
...
gibt es wenig Unterrichtszeit in der die KanditadInnen auf die
Prüfung vorbereitet werden können. Dieses Skript möchte den theoretischen Prüfungsstoff im 
Fach Musik präsentieren, damit ebendiese Quereinsteiger sich zum Teil selbstständig 
auf die Musikprüfung vorbereiten können.

\section{Zur Prüfung}
Während der Prüfung in das Fach Musik muss man nachweisen, dass man in der Lage ist,
sich ein Kinderlied anzueignen, 
dieses zum Klingen zu bringen (sowohl singend als spielend), zu begleiten und mit 
Kindern zu singen oder musizieren. Dazu sollen
Sprache (der Liedtext) und Bewegung (Tanz, Spiele) sinnvoll eingesetzt werden.

Damit der/die KandidatIn sich ein neues Lied aneignen kann, wird in diesem Skript zuerst das 
Notenlesen behandelt. Danach wird auf 
einfache Begleitungsarten eingegangen. Die Instrumentenkunde umfasst das kleine
Schlagwerk. Abschließend gibt es noch einige Tipps für das Musizieren
mit Kindern in Kindergartenalter.
...
In unserem Tonsystem gibt es sieben Töne. Von diesen sieben Töne, die auch Stamm\-tö\-ne 
\comment{Stammtöne}
genannt werden, 
...
findet man acht Töne. Dieser Abstand \emph{c --- c} nennt man 
\emph{Oktave}\footnote{von dem lateinischen \glqq 
	octavus\grqq -- der achte}.
\comment{Oktave}

Es gibt folglich mehrere \emph{c's} oder \emph{g's}. 
...
zu der \emph{eingestrichene Oktave} --- man sagt: 
...
\image{lilypond/oktaven.png}{ein Teil der kleinen Oktave, ein- und zweigestrichenen Oktave}{oktaven}{13}

Stimmen von Kindern im Kindergartenalter %
\footnote{vgl. Ernst, Manfred: 
Praxis Singen mit Kindern, S. 13}
...
zwei Tönen ein Ganztonschritt. Jedoch zwischen dem \emph{e} und 
...
Ein \emph{Kreuz} macht eine Note um einen Halbtonschritt höher. Ein \emph{b} macht eine 
Note um einen Halbtonschritt tiefer. Ein Kreuz sieht so aus: $\sharp$, und ein b: $\flat$.
...
Ein \glqq tiefergelegten\grqq\  Ton bekommt die Silbe 
...
historische Gründe hat.\footnote{Es würde den Ramen dieses Skripts sprengen, 
näher auf diese Gründe einzugehen.}

Beachte, dass man das Versetzungszeichen \emph{vor} der Note schreibt. 
Siehe auch \autoref{verszeichen} 
\image{lilypond/versetzung.png}{Notenschrift mit Versetzungszeichen}{verszeichen}{5}
...
Regel nicht benutzt, auch wenn sie theoretisch korrekt wären.
...
\footnote{Es gibt noch den Brevis, der zweimal so lange dauert wie eine ganze 
Note, und die Longa die viermal so lange dauert. Diese werden seit dem 19. Jahrhundert 
nicht mehr benutzt, und haben für die moderne Musikpraxis keine Bedeutung. Notenwerte 
kürzer als die 32-tel Note gibt es zwar, sind jedoch nicht Gegenstand der Prüfung.}
...
\begin{table}[ht]
	...
	\label{notenwerte}
\end{table}
Diese Längenangaben sagen nichts darüber aus, wie lange (in Sekunden) eine Note dauert. 
Es ist lediglich festgelegt, dass z.B. eine ganze Note viermal so lange dauert wie eine
...
werden, falls es mehrere hintereinander gibt, oft zusammengefasst. Die Fähnchen werden 
...
die Noten die mit einer \glqq 1\grqq\ markiert sind zu betonen. Dadurch, dass diese
